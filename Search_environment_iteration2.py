...
class Swarm(object):
	...
	def check_in_FOV(self, pos):
		...
		#var = np.sum([p if polygon.contains(p) for p in point])
		return var
	...
	def check_for_duplicates(self, targets):
		var = []
		for t in targets:
			if t not in self.detected_targets_list:
				var.append(t)
	...
	def step(self, v):
		...
		for i, pos1 in enumerate(self.pos):
			...
			detected_targets = self.check_for_duplicates(self.check_in_FOV(pos1))
			self.detected_targets_list += detected_targets
			
			reward += self.target_reward * len(detected_targets)

			reward += self.goal_reward/(1 + 0.1*abs(goal_distance-self.wp_rad))
