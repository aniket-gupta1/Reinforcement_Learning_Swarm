...
from Search_environment_iteration2 import Swarm
...
name = f'Search_env_1_{time_var.tm_mon}_{time_var.tm_mday}_{time_var.tm_hour}_{time_var.tm_min}_{time_var.tm_sec}'
...
class TD3():
	...
	def __init__(self, env_name, env, Actor_net, Critic_net1, Critic_net2, act_dim, obs_dim ,lam = 0.97,decay = False,
				...
				value_train_iterations = 5, memory_size = 100000, polyak_const = 0.995, minibatch_size = 1000):
	...
	def train(self, episodes):
		...
		for episode in range(episodes):
			...
			if episode%10 == 0 and episode != 0:
				self.Actor.save_weights(f"Weights_S/{name}/Episode{episode}/actor.ckpt")
				self.Critic1.save_weights(f"Weights_S/{name}/Episode{episode}/critic1.ckpt")
				self.Critic2.save_weights(f"Weights_S/{name}/Episode{episode}/critic2.ckpt")
